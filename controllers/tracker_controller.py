...
def add_peer_to_file(torrent, peer_id, pieces_idx):
    ...
    if file_data:
        collection.update_one(
            {"metainfo_id": ObjectId(metainfo_id)},
            {"$push": {
                "peers_info": {"peer_id": ObjectId(peer_id), "pieces": pieces_idx}
            }}
        )
...
def upload_file(file_path, peer_id):
    try:
        pieces, pieces_arr, pieces_idx = torrent_create.create_pieces(file_path, 512000)
        file_path.seek(0, 2)  # Seek the end of the file
        file_length = file_path.tell()  # Get the file length
        file_path.seek(0, 0)  # Go back to the beginning of the file

        output_file = f"{file_path.filename}.torrent"
        info_hash = torrent_create.create_info_hash(file_path.filename, 512000, pieces, file_length)
...
def update_peer_shared_files(peer_id, metainfo_id, pieces_arr):
    ...
    collection.update_one(
        ...
        {
            "$addToSet": {"piece_info": piece_data},
        }
    )
...
def get_new_piece(magnet_link, peer_id):
    ...
    pieces = peer_controller.request_piece_from_peers(peer_list, pieces_index, torrent_data, available_pieces)

    pieces_arr = [(pieces[i], i) for i in range(len(pieces))]
